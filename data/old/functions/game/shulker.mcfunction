...
execute if entity @s[scores={health=0..,Hurted=..29}] if score #NumHealth settings matches 1 run function game:game/update_name_num
execute if entity @s[scores={health=0..,Hurted=..29}] if score #NumHealth settings matches 0 run function game:game/update_name_sym

execute if entity @s[tag=BlueBed,scores={health=0}] run tellraw @a [{"text":"Шалкер Жёлтой команды был уничтожен","color":"aqua"}]
execute if entity @s[tag=YellowBed,scores={health=0}] run tellraw @a [{"text":"Шалкер Синей команды был уничтожен","color":"yellow"}]
