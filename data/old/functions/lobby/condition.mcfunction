...
execute if score #TCount var matches ..1 run tellraw @a {"text":"Недостаточно игроков для начала","color":"red"}
execute if score #TCount var matches 2.. if score #PCount var matches 1.. run tellraw @a {"text":"Некоторые игроки не выбрали команду","color":"red"}
execute if score #TCount var matches 2.. if score #BCount var matches 0 if score #PCount var matches 0 run tellraw @a {"text":"Синяя команда пустая","color":"red"}
execute if score #TCount var matches 2.. if score #YCount var matches 0 if score #PCount var matches 0 run tellraw @a {"text":"Жёлтая команда пустая","color":"red"}

execute unless score #State var matches 4 as @a at @s run playsound minecraft:entity.item.break master @s ~ ~ ~ 0.6 1 1
execute unless score #State var matches 4 run scoreboard players set #State var 2
execute if score #FastStart settings matches 0 if score #State var matches 4 run scoreboard players set #Countdown var 10
execute if score #FastStart settings matches 1 if score #State var matches 4 run scoreboard players set #Countdown var 5
