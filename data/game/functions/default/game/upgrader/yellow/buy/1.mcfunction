...
execute at @e[type=minecraft:shulker,tag=yBed,limit=1] run particle minecraft:nautilus ~ ~1.2 ~ 0.1 0.2 0.1 1 100 force
execute as @e[type=minecraft:shulker,tag=yBed,limit=1] run function #game:shulker/update_name
#Translate
tellraw @a[team=yellow] {"storage": "game:lang", "nbt": "Upgrader.ShulkerPlusHP", "interpret": true}
